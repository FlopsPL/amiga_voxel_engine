...
#define wait_less 0x0023
#define wait_greater 0x8023
...
unsigned short* build_8x8_c2p_copper_list() {
  ...
  unsigned short color_change_tail = 0x0f00; /// Black color 0RGB
 ...
 *memoryAddress++ = 0x0033; /// Wait for 0x37px. Here starts min loop (8 times same line).
   ...
   *memoryAddress++ = skip_command_tail;
    ...
    *memoryAddress++ = copper_list_reset_tail; /// It is skiped if raster line is after line set in above skip command.
    ...
    // *memoryAddress++ = 0x000f;; /// Wait for 0x37px. Here starts min loop (8 times same line).
    // *memoryAddress++ = wait_command_tail_px;
